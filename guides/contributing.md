...
- [`Lotus.Preflight.Relations`](../lib/lotus/preflight/relations.ex) — The preflight outcome type: the `{schema, table}` list a statement provably touches, or an `:unrestricted` / `:skipped` tuple. `to_list/1` narrows it for column policy lookup. Its process-dictionary functions are deprecated and go away in v2.0.
...
- `Lotus.SQL.*` (`lib/lotus/sql/`) — Low-level SQL helpers (sanitizer, identifier quoting, filter/sort injectors, validator, transformer). They do not depend on Ecto: any SQL adapter, Ecto-backed or not, can reuse them. They are not part of the universal adapter contract.
...
│  • Miss → authorize, then execute inside the cache write below       │
└─────────────────────────────────────────────────────────────────────┘
                               │ miss / :bypass / :refresh
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
│ Lotus.Runner.authorize_phases/3, then execute_authorized/4           │
...
│  4. Middleware.run(:before_execute, _) — relations in, assigns out   │
│  5. Lotus.Cache.get_or_store/4, around the execution only:           │
│     Adapter.transaction (read-only) → Adapter.execute_query,         │
│     then column policy enforcement (omit / mask / error)             │
│  6. Telemetry.query_stop / query_exception                           │
│  → the cache stores %{result:, relations:}; the assigns stay with    │
│    this call                                                         │
└─────────────────────────────────────────────────────────────────────┘
                               │
                               ▼
...
│    with the assigns :before_execute left on this call                │
...
- **Filters and sorts** are injected through the adapter, not concatenated naively — see `Lotus.SQL.FilterInjector` and `SortInjector`. An adapter must declare the operators it handles via `supported_filter_operators/1`; anything else raises `Lotus.UnsupportedOperatorError`.
...
- **Caching** is optional. When no cache adapter is configured, `Lotus.Cache` is a pass-through and the fetcher always runs. The cache wraps the execution only, and stores the relations preflight found alongside the result, so all three query events fire on a hit — `:before_execute` gates on the stored relations. On a miss, sanitization, preflight and `:before_execute` run before the cache write, so the assigns `:before_execute` leaves for `:after_query` never reach the entry. `Lotus.Runner.run_statement/3` composes the same phases with no cache between them. What a hit skips is sanitization, preflight and column visibility, all of which read `:scope`, which is in the key.
- **Preflight** is skipped when `needs_preflight?/2` returns false (the Ecto adapter keeps the `EXPLAIN` / `SHOW` / `PRAGMA` heuristic internally). `Lotus.Preflight.analyze/4` returns the relations it discovers as a value, and the runner carries them down the pipeline — to `:before_execute` middleware, and to column visibility policy lookup — without re-parsing the statement or touching the process dictionary. An adapter that cannot enumerate resources returns `{:unrestricted, reason}`, which is blocked unless the operator opts in with `:allow_unrestricted_resources`.
...
6. **Session state is scoped and explicit.** Per-request state like statement timeouts and search paths is set by the adapter at the start of a transaction; there is no hidden global state. The preflight outcome travels as a value through `Lotus.Runner`, not through the process dictionary, so it cannot leak into a reused process or vanish across a `Task`.
