...
`Lotus.Notifier` relays notifications between nodes over OTP process groups: the `:pg` module of the `kernel` application, which has nothing to do with PostgreSQL. It starts as the first child of `Lotus.Supervisor` and needs no configuration: connected nodes share it automatically. Core uses two topics:
...
The node that makes the change applies it locally first and returns, then the others hear about it. Delivery is a message send between connected nodes: a node that is partitioned away misses it and relies on cache TTLs and its next reconcile.
...
`c:Lotus.Cache.Adapter.scope/0` tells `Lotus.Cache` how far one call on the adapter reaches. `:node` means the store is local to each node, so `delete/1` and `invalidate_tags/1` are relayed over `Lotus.Notifier`. `:cluster` means one call already reaches every node and nothing is relayed. `Lotus.Cache.scope/0` returns the answer for the configured adapter.
